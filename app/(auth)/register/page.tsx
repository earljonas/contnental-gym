...
export default function RegisterPage() {
  ...
  const handleVerify = () => {
    if (form.otp.join("").length < 6) { setErrors({ otp: "Enter the 6-digit code" }); return; }
    setLoading(true);
    setTimeout(() => { setLoading(false); next(); }, 1200);
  };
            ...
            {steps.map((label, i) => (
              <div key={label} className="flex items-center">
                <div className="flex flex-col items-center">
                  <div
                    className={`flex h-8 w-8 items-center justify-center text-[12px] font-medium transition-colors ${
                      i <= step
                  ...
                  <span className={`mt-2 text-[10px] uppercase tracking-[0.1em] ${i <= step ? "text-white" : "text-text-muted"}`}>
                    {label}
                  </span>
                </div>
                {i < steps.length - 1 && (
                  <div
                    className={`mx-2 h-px w-8 sm:w-14 ${i < step ? "bg-white" : "bg-border-subtle"}`}
        ...
        >
          ...
          {step === 0 && (
            <div>
              ...
              <button onClick={handleAccountNext} className="mt-8 flex h-12 w-full items-center justify-center border border-white bg-white text-[12px] font-medium uppercase tracking-[0.15em] text-black transition-all hover:bg-white/90">
                CONTINUE
              </button>

              <p className="mt-6 text-center text-[14px] text-text-secondary">
                Already have an account?{" "}
                <Link href="/login" className="font-medium text-white underline underline-offset-4 transition-colors hover:text-gold">
                  Sign in
                </Link>
              </p>
            </div>
          ...
          {step === 1 && (
            <div>
              ...
              <div className="space-y-3">
                {plans.map((plan) => (
                  <button
                    key={plan.id}
                    type="button"
                    onClick={() => { setForm((p) => ({ ...p, plan: plan.id })); setErrors({}); }}
                    className={`relative w-full border p-5 text-left transition-all ${
                      form.plan === plan.id
                        ? "border-white bg-white/5"
                        : "border-border-subtle bg-surface hover:border-white/30"
                    }`}
                  >
                    {plan.popular && (
                      <span className="absolute -top-2.5 right-4 bg-white px-2 py-0.5 text-[9px] font-semibold uppercase tracking-[0.1em] text-black">
                        POPULAR
                      </span>
                    )}
                    <div className="flex items-center justify-between">
              ...
              <div className="mt-8 flex gap-3">
                <button onClick={back} className="flex h-12 flex-1 items-center justify-center border border-border-subtle text-[12px] font-medium uppercase tracking-[0.15em] text-white transition-colors hover:border-white hover:bg-white/5">
                  BACK
                </button>
          ...
          {step === 2 && (
            <div>
              ...
              <div className="flex justify-center gap-3">
                {form.otp.map((digit, i) => (
                  <Input
                    ...
                    className={`h-14 w-12 rounded-none border-border-subtle bg-surface text-center text-lg font-medium text-white focus-visible:border-white focus-visible:ring-0 ${errors.otp ? "border-red-500/60" : ""}`}
              ...
              <p className="mt-6 text-center text-[13px] text-text-secondary">
                Didn&apos;t receive it?{" "}
                <button className="text-white underline underline-offset-4 transition-colors hover:text-gold">Resend code</button>
              </p>
          ...
          {step === 3 && (
            <div>
              ...
              <p className="mt-3 max-w-sm text-[14px] leading-relaxed text-text-secondary">
                Your account has been created. Your membership will be
                activated once payment is confirmed.
              </p>

              {/* Summary */}
              <div className="mt-8 border border-border-subtle bg-surface p-6">
                <h3 className="mb-4 text-[11px] font-medium uppercase tracking-[0.15em] text-text-secondary">
                ...
                <div className="space-y-3 text-[14px]">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium uppercase text-gold">{plans.find((p) => p.id === form.plan)?.name}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-text-secondary">Price</span>
                    <span className="text-white">{plans.find((p) => p.id === form.plan)?.price}{plans.find((p) => p.id === form.plan)?.period}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-text-secondary">Status</span>
                    <span className="text-[12px] uppercase tracking-wider text-yellow-500">Pending Activation</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-4 border border-border-subtle bg-surface/50 p-5">
                <h4 className="mb-3 text-[11px] font-medium uppercase tracking-[0.15em] text-text-secondary">
                  NEXT STEPS
                </h4>
                <ul className="space-y-2 text-[13px] text-text-secondary">
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 text-gold">1.</span>
                    Visit Continental Fitness Gym in Davao City
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="mt-0.5 text-gold">2.</span>
                    Pay at the front desk or via GCash
                  </li>
                  <li className="flex items-start gap-2">
              ...
              <Link href="/login" className="mt-8 flex h-12 w-full items-center justify-center border border-white bg-white text-[12px] font-medium uppercase tracking-[0.15em] text-black transition-all hover:bg-white/90">
                GO TO LOGIN
              </Link>
            </div>
          )}
